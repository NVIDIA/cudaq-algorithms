...
**Representation Record** for the injectable one-register preparation
kernel and a **Capability Record** for unitary state preparation — plus direct
links to the two concrete primitive records, which instantiate the
primitive-only headings once each and own their contracts. `conventions.md`
owns the cross-cutting layout, ownership, and validation conventions both
providers depend on.
...
| --- | --- |
| [state-preparation-givens.md](state-preparation-givens.md) | Slater determinant from an orthonormal orbital-coefficient matrix, through a Givens rotation schedule |
...
Each is one independently selectable scientific contract — a different input
object, mathematical definition, validation and rejection behavior, and its own
resource estimators — not a variant or spelling of the other. Inputs, outputs,
validation, resources, and provenance detail live only in those two files; a
task needing both contracts reads both records directly, with no third-file
chain.

## Family scope and source provenance

- Owner: CUDA-Q Algorithms Team.
- Family scope: emit a one-argument device kernel that prepares a specified
  many-fermion state on a register in `|0...0>`, for injection into the packaged
  consumers. Choosing or optimizing amplitudes is a consumer workflow, out of
  scope (`SKILL.md`).
...
- Package/CUDA-Q versions verified: **unverified.** The declared environment is
  ...
  pinned at `b28cf0f6f2d9387f12ca81b6824b8833a38530af` (`.cudaq_version`), but
  no test was executed for this family.
- Commit/date last verified: `61ac072d7823481ad0d303dac84d8ee29a9a8cd0`
  (2026-09-03). Both primitive records carry the same stamp.
- Lifecycle: draft, for this file and for both primitive records.
- **Evidence rule for this family.** Every test named in any of the three files
  is *cited repository evidence* — a committed assertion read at the cited
  commit, labeled `derived` — never a fresh measurement in this session.
...
- Observable symptom of a misinterpretation: a multi-argument kernel offered
  here will not launch, or the consumer factory silently stays in its
  `cudaq.State` mode; a wrong-width kernel fails at launch (detail unverified).
- Unsupported or ambiguous forms: multi-argument device kernels — a different
  representation, not a variant of this one; kernels that allocate ancillas,
...
- Stable ID: `cudaq-algorithms.state-preparation.unitary.v1`. This deviates from
  `architecture.md`'s `cudaq-algorithms.<capability-name>.v<major>` unless
  `<capability-name>` is read as the dotted `state-preparation.unitary`; the
  final ID, and whether `.v1` is warranted with no second version in sight,
  remain an **open** owner decision.
...
- Semantic invariants, `derived` from the `state_prep` call sites and the
  ...
  2. **Exact width match, unenforced at factory time.** The preparation acts
     only on the register handed to it, of width `num_qubits` (Trotter) or
     `num_system` (encodings). The documentation states this is not verifiable
     at factory time, so a mismatch fails at launch.
...
- Register or shape geometry and ownership: geometry is invariant 2. Ownership
  here is **caller-owned in the current source only**: the consumer factory
  that calls the preparation kernel allocates the register, hands it over fresh
  in `|0...0>`, and expects exactly its own system width. That is a `derived`
  description of the implementation at the cited commit, **explicitly not a
  universal or future policy.** Whether the caller or the primitive should own
  system and ancilla registers in general is an **open** question in the
  taxonomy design record outside this package (`conventions.md` repeats the
  scope limit); do not present the current behavior as a library-wide guarantee,
...
- Providers, with source paths: `slater_determinant_kernel`
  ...
  [state-preparation-givens.md](state-preparation-givens.md));
...
- Promotion criteria to a public protocol or compiler IR operation, and the
  explicit decision still required: (a) a provider outside the current two that
  exercises the same boundary without widening it, (b) a resolved
  register-ownership policy, (c) characterized behavior for the conditions
  marked `unverified` below, and (d) an explicit team decision recorded with an
  owner. None of the four holds at the cited commit; no promotion is proposed.
...
| Width-mismatch failure detail | unverified | documentation states a mismatch fails at launch, but no source or test states the error type or message and no test exercises a mismatched width |
...
| --- | --- |
| `state-preparation-provider-selection` | this file (scope, Representation Record, Capability Record) **and both** primitive records, whose Classification, Scientific contract, and Inputs sections carry the per-provider answer |
...
No case requires a third hop: each is answerable from this file plus a record it
links directly. None has been run with or without the skill, so this table is
declared coverage, not validated coverage; `evals/EVAL.md` owns the procedure.
