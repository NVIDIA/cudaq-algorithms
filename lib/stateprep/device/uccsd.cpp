...
get_uccsd_excitations(std::size_t num_qubits, std::size_t num_electrons,
  ...
  auto numSpatialOrbs = num_qubits / 2;
  std::vector<std::size_t> occupiedAlpha, virtualAlpha, occupiedBeta,
      virtualBeta;
  if (spin > 0) {
    auto n_occupied_beta =
        static_cast<std::size_t>(std::floor((float)(num_electrons - spin) / 2));
    auto n_occupied_alpha = num_electrons - n_occupied_beta;
    auto n_virtual_alpha = numSpatialOrbs - n_occupied_alpha;
    auto n_virtual_beta = numSpatialOrbs - n_occupied_beta;

    for (std::size_t i = 0; i < n_occupied_alpha; ++i)
      occupiedAlpha.push_back(i * 2);

    for (std::size_t i = 0; i < n_virtual_alpha; ++i)
      virtualAlpha.push_back(i * 2 + 2 * n_occupied_alpha);

    for (std::size_t i = 0; i < n_occupied_beta; ++i)
      occupiedBeta.push_back(i * 2 + 1);

    for (std::size_t i = 0; i < n_virtual_beta; ++i)
      virtualBeta.push_back(i * 2 + 2 * n_occupied_beta + 1);
  } else if (num_electrons % 2 == 0 && spin == 0) {
    auto numOccupied =
        static_cast<std::size_t>(std::floor((float)num_electrons / 2));
    ...
    for (std::size_t i = 0; i < numVirtual; ++i)
      virtualAlpha.push_back(i * 2 + num_electrons);

    for (std::size_t i = 0; i < numOccupied; ++i)
      occupiedBeta.push_back(i * 2 + 1);

    for (std::size_t i = 0; i < numVirtual; ++i)
      virtualBeta.push_back(i * 2 + num_electrons + 1);

  } else
    throw std::runtime_error("Incorrect spin multiplicity. Number of electrons "
                             "is odd but spin is 0 " +
                             std::to_string(num_electrons) + ", " +
                             std::to_string(spin));
