...
std::complex<double>
two_body_coef(const std::vector<std::complex<double>> &two_body, std::size_t n,
              std::size_t p, std::size_t q, std::size_t r, std::size_t s) {
  return two_body_at(two_body, n, p, q, r, s) -
         two_body_at(two_body, n, q, p, r, s) -
         two_body_at(two_body, n, p, q, s, r) +
         two_body_at(two_body, n, q, p, s, r);
}
...
cudaq::spin_op bravyi_kitaev(const std::vector<std::complex<double>> &one_body,
  ...
  auto nqubits = num_spin_orbitals;
  cudaq::spin_op spin_hamiltonian = cudaq::spin_op::empty();

  double constant_term = scalar_offset;
  for (std::size_t p = 0; p < nqubits; p++) {
    if (std::abs(one_body_at(one_body, nqubits, p, p)) > 0.0) {
      spin_hamiltonian += seeley_richard_love(
          p, p, one_body_at(one_body, nqubits, p, p), nqubits);
    }
    for (std::size_t q = 0; q < p; q++) {
      if (std::abs(one_body_at(one_body, nqubits, p, q)) > 0.0) {
        spin_hamiltonian += seeley_richard_love(
            p, q, one_body_at(one_body, nqubits, p, q), nqubits);
        spin_hamiltonian += seeley_richard_love(
            q, p, std::conj(one_body_at(one_body, nqubits, p, q)), nqubits);
      }
      auto coef = 0.25 * two_body_coef(two_body, nqubits, p, q, q, p);
      if (std::abs(coef) > 0.0) {
  ...
  for (std::size_t p = 0; p < nqubits; p++) {
    for (std::size_t q = 0; q < nqubits; q++) {
      for (std::size_t r = 0; r < q; r++) {
        if ((p != q) and (p != r)) {
          auto coef = two_body_coef(two_body, nqubits, p, q, r, p);
          if (std::abs(coef) > 0.0) {
    ...
    for (std::size_t q = 0; q < p; q++) {
      for (std::size_t r = 0; r < q; r++) {
        for (std::size_t s = 0; s < r; s++) {
          auto coef_pqrs = -two_body_coef(two_body, nqubits, p, q, r, s);
          if (std::abs(coef_pqrs) > 0.0) {
            spin_hamiltonian +=
                hermitian_one_body_product(p, q, r, s, coef_pqrs, nqubits);
          }
          auto coef_prqs = -two_body_coef(two_body, nqubits, p, r, q, s);
          if (std::abs(coef_prqs) > 0.0) {
            spin_hamiltonian +=
                hermitian_one_body_product(p, r, q, s, coef_prqs, nqubits);
          }
          auto coef_psqr = -two_body_coef(two_body, nqubits, p, s, q, r);
...
cudaq::spin_op bravyi_kitaev(const std::vector<std::complex<double>> &one_body,
                             std::size_t num_spin_orbitals,
                             double scalar_offset, double tolerance) {
  return bravyi_kitaev(one_body, {}, num_spin_orbitals, scalar_offset,
                       tolerance);
}
